...
//  Done - @TODO Cleanup (delete) any other data files associated with this user 
                        ...
                        _data.delete('checks',checkId, function(err){
                          ...
                          if(checksDeleted == checksToDelete){
                            if(!deletionErrors){
                              ...
                              callback(500, {'Error' : 'Errors encountered while attempting to delete all of the user\'s checks. All checks may not have been deleted from teh system successfully'});
                            }
                          }
                        });
                      });
...
handlers._tokens.post = function(data,callback){
  ...
  if(phone && password){
    // Lookup for the user who matches that phone number
    _data.read('users',phone,function(err,userData){
      if(!err && userData){
...
// Verify if a current token id is currently valid for a given user
